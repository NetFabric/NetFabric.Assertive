...
- `BeObservable<TActualItem>()` - asserts that the type `TActual`, passed in to `Must<TActual>()`, is an observable that returns a stream of items of type `TActualItem`.

- `NotShareState()` - asserts that the enumerators generated by the actual enumerable object do not share state.
